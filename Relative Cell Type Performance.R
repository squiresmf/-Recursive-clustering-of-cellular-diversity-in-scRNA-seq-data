...
if (!file.exists('cell_names_list.rds')) {
  for (reference_name in c("human_PBMC", "human_adipose", "human_tonsil", "human_fetus")) {
    reference_example = readRDS(paste0('reference_', reference_name, '_integrated_0.015_0_cR.rds'))
    for (annotation_level in seq(2)) {
      annotation_level_final = annotation_levels[[reference_name]] - (2 - annotation_level)
      cell_names_list[[reference_name]][[annotation_level]] = sort(unique(reference_example@meta.data[[paste0('celltype.l', annotation_level_final)]]))
    }
    for (cell_type_l2 in cell_names_list[[reference_name]][[2]]) {
      cell_type_l1 = reference_example@meta.data[[paste0('celltype.l', annotation_level_final-1)]][[match(cell_type_l2, reference_example@meta.data[[paste0('celltype.l', annotation_level_final)]])]]
      ...
      for (cell_type_idx in seq_along(c(cell_type_l1, cell_type_l2))) {
        ...
        num_cells_cell_type = sum(reference_example@meta.data[[paste0('celltype.l', cell_type_idx)]] == cell_type)
        num_cells_total = length(reference_example@meta.data[[paste0('celltype.l1')]])
...
plot = ggplot(results_df_all, aes(x = value_seurat, y = value_iterative)) +
  ...
  theme(panel.background = element_rect(fill = 'white', colour = 'white'), plot.background = element_rect(fill = 'white', colour = 'white'), panel.grid.major = element_blank(), panel.grid.minor = element_blank(), panel.border = element_rect(color = "grey", fill = NA, size = 1),
    ...
    axis.title.y = element_text(size = 9, vjust = -15, angle = 90), legend.key.width = unit(1.5, "cm"), plot.margin = margin(0, 0, 0, 0, "pt"), legend.margin = margin(0, 20, 0, 0),
          legend.title = element_blank(), legend.position = 'bottom', legend.spacing.x = unit(0.05, "cm"), axis.title.x = element_text(size = 9, margin = margin(t = 12, unit = "pt"))

  ) +
  labs(x = "Single-Pass Cell Type Cluster Purity", y = "Recursive Cell Type Cluster Purity") +
  facet_grid(Annotation ~ Reference, switch = "y")


ggsave('cell type mean cluster purity plot.png', plot, dpi = 275, width = 9, height = 5, device = 'png')

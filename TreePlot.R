...
GetRadialTreeFromDataframe <- function(df, tree_comparisons, val_name, tree_level, gravities, title_extra = '') {
  radial_tree_m = df
  ...
  for (tree_comparison_idx in seq_along(tree_comparisons)){
    ...
    saveNetwork(radial_tree, paste0(tree_name, '.html'), selfcontained = FALSE)
    webshot(paste0(tree_name, '.html'), paste0(tree_name, '.png'), vwidth = (tree_level + 2)*100, vheight = (level + 2)*100, zoom = 3)

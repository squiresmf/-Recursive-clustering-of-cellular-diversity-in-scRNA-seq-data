    ...
    for (level in seq(0, m_level)){
      ...
      for (test in tests) {
        ...
        for (alpha in c(0.05, 0.01)){
          for (adjusting in c('adjusted', 'unadjusted')){
            if (adjusting == 'adjusted'){
              temp_p_value_vector = as.vector(temp_p_value_table)
              m = matrix(holm(temp_p_value_vector, alpha = 0.05, silent = TRUE)$adjPValues, nrow = nrow(temp_p_value_table), ncol = ncol(temp_p_value_table), byrow = FALSE, dimnames = list(rownames(temp_p_value_table), colnames(temp_p_value_table)))
            } else {
            ...
            p_value_table_plots[[cluster_type]][[metric]][[level_name]][[test]][[paste0(alpha)]][[adjusting]] =
             ...
             coord_flip() + scale_x_discrete(limits=rev) + scale_y_discrete() +

...
for (max_level in 2){
 ...
 m_rows = list()
   for (cluster_name in level_ordered_cluster_names[2:length(level_ordered_cluster_names)]){
    ...
    pca_similarity_factor = sum(diag(t(root_eigenvectors) %*% cluster_eigenvectors %*% t(cluster_eigenvectors) %*% root_eigenvectors)) / ncol(cluster_eigenvectors_df)
...
pca_similarity_factor = sum(diag(t(pca.results1$v) %*% pca.results2$v %*% t(pca.results2$v) %*% pca.results1$v)) / npcs
...
pca_similarity_factor2 = sum(diag(t(pca.results1$v) %*% pca.results2$v %*% t(pca.results2$v) %*% pca.results1$v)) / npcs
